...
$(function () {
    $("#btnPass").click(function () {
        $.post("/user/getByProcessRole.bms",{isDbChange:prInfo.isDbchange,processName:prInfo.newProcessName},function (data) {
            var datas = [];
            var i = 0;
     ...
     * 确认拒绝
     */
    $("#refuseQuerenBtn").click(function () {
        $.post("/pr/refuse.bms",{id:prInfo.id,refuseReason:$("#refuseReason").val(),isDbchange:prInfo.isDbchange},function (data) {
            if(data.code == "0000"){
                toastrsuccess(data.msg);
                loadTable();
                reloadRightInfo(prInfo.id);
                closeRightWindow();
            } else {
                toastrerror(data.msg);
            }
            $("#refuseWindow").modal('hide');
        },"json");
    });
    
    $("#passQuerenBtn").click(function () {
        $.post("/pr/pass.bms",{id:prInfo.id,agent:$("#agentSelect").select2('val'),isDbchange:prInfo.isDbchange},function (data) {
            if(data.code == "0000"){
                toastrsuccess(data.msg);
                loadTable();
                reloadRightInfo(prInfo.id);
    ...
    $('#processTable').bootstrapTable({
        url: '/pr/pageAssignedToMeRelease.bms?projectId=' + localStorage.getItem("stotage_project_id"),
